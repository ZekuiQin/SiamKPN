...
if __name__ == '__main__':
    ...
    # load model
    for current_epoch in range(args.start_epoch,args.end_epoch+1):
        model = ModelBuilder()
        snapshot = 'snapshot/checkpoint_e' + str(current_epoch) + '.pth'
        model = load_pretrain(model, snapshot).cuda().eval()

        # build tracker
        tracker = build_tracker(model)

        model_name = 'checkpoint_e' + str(current_epoch)
        benchmark_path = os.path.join('hp_search_result_epoch', args.dataset)

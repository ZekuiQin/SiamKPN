...
def build_opt_lr(model, current_epoch=0):
    ...
    trainable_params += [{'params': model.kpn_head.parameters(),
                          'lr': cfg.TRAIN.BASE_LR}]
...
def log_grads(model, tb_writer, tb_index):
    ...
    tot_norm = feature_norm + kpn_norm
    tot_norm = tot_norm ** 0.5
    feature_norm = feature_norm ** 0.5
    kpn_norm = kpn_norm ** 0.5

    tb_writer.add_scalar('grad/tot', tot_norm, tb_index)
    tb_writer.add_scalar('grad/feature', feature_norm, tb_index)
    tb_writer.add_scalar('grad/kpn', kpn_norm, tb_index)
    ...
    for idx, data in enumerate(train_loader):
        ...
        if rank == 0:
            ...
            if (idx+1) % cfg.TRAIN.PRINT_FREQ == 0:
                info = "Epoch: [{}][{}/{}] lr: {:.6f} true_lr: {:.6f}\n".format(
                            epoch+1, (idx+1) % num_per_epoch,
                            num_per_epoch, cur_lr, epoch_lr[1])
